ValueSet: NIHSS9Sprog
Title: "NIHSS - 9. Sprog"
Description: "The National Institute of Health Stroke Scale (NIHSS) - 9. Sprog (bed patienten beskrive billeder, benævne genstande, læse sætninger)"

* ^status = #active
* ^publisher = "RKKP"
* ^date = "2022-06-27T00:00:00+02:00"

* NIHSS9Language#0 "0 = Ingen afasi"
* NIHSS9Language#1 "1 = Let til moderat afasi"
* NIHSS9Language#2 "2 = Svær afasi"
* NIHSS9Language#3 "3 = Global afasi"